...
| A declared name, separate from the Rust type | **Neither has one.** LWIM's saved bindings key on the enum variant, BEI's on the type path, so a rename — or, for BEI, a module move — orphans what a player saved. D8 requires a `PATH` that a refactor cannot touch, and which doubles as the label's localization key (§1, §17). |
