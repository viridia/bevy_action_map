...
*possible*: an input frame event carries the device it came from, a context entity can be paired to
a device through a sibling `Paired` component, and an unpaired device drives nothing — `apply_frame`
filters its read by device before anything else touches it. Absence of `Paired` is exactly today's
behavior, so this costs a single-player game nothing.

This alone satisfies R15.3's own text ("a device's input must not reach a player who does not own
it... enforced at L1/L2") — a paired instance's fold simply never sees another device's raw events.
It also fixes, as a side effect, `held_gamepad_buttons`/`held_gamepad_axes` being one flat map across
every connected pad: a disconnect today clears all of them at once, invisible only because nothing
in tree has two.

- **Inherited from chunk 14:** R22.1's fifth obstacle, "device not owned by this player", answered
  by `Paired` existing to ask, though wiring it into the diagnostic itself is not this chunk's job.
...
- **Review surface, resolved:** pairing is a sibling component next to `InputContextState`, not a
  field on it and not a filter on the plan — generic-free, so chunk 66's join flow can ask "is this
  device claimed by anything" without enumerating every declared context type.
- **Not this chunk:** owner-scoping `ConsumedControls` and the exclusion ceiling (chunk 61's own
  inheritance, and R15.3's cross-context consumption case) is real but nothing in tree needs it —
  Split Friction (chunk 27) declines rebinding UI and its device-selection screen is chunk 66's own
  `JoinSession`, not an `InputContext` — so it is written to the deferred table below rather than
  built ahead of a need. The design it would use if built is recorded there too, so it does not need
  re-deriving: a claim visible only if global or made by the viewer's own device; an exclusive
  context's shadow scoped to its own pairing unless left unpaired, which is also Split Fiction's own
  precedent for a *deliberate* mutual lockout.

### 66. The join gesture

The join gesture chunk 26's own text used to promise: an app-driven query for "a device that just
pressed something and is not yet claimed" (R15.4), the same read of L1 that capture uses in chunk 20
— reusing capture's own `arrival()` classifier rather than a second one. Depends on chunk 26 for
`Paired`/`DeviceHandle`.

- **Verified by:** chunk 27's device-selection screen — press anything on a pad or a key on the
  keyboard and that mapping is yours.
- **Prior art:** Unity's `PlayerInputManager` join-on-button-press, Steam Input, Unreal's local
  player subsystem. The primitive answers one fact — this device, previously unclaimed, just pressed
  something — and leaves slot count, screen layout, and any avatar step entirely to the app, the
  same way this crate leaves presentation to `examples/common/` elsewhere.
