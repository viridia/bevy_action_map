...
| **A cheaper watch on Steam's layout** | **a profile showing the Steam build's origin poll costs a frame something**, or a game with enough actions that it plainly would. `poll` asks every action's origins every frame and compares, because `steamworks` 0.13 has no safe layout-change callback (`docs/steam.md` S30). Prompts can afford to lag, so the options are, cheapest first: ask on the events that can change the answer (the window gaining focus, which S30 found is when a rebind shows, and a pad connecting or going); check one action per frame in rotation; ask on a timer; skip frames with no prompt on screen; or receive `SteamInputConfigurationLoaded_t` through an `unsafe impl Callback`, which first needs measuring whether it fires for a rebind in the panel. Events with a slow rotation behind them combine well. The comment in `poll` points a reader at these |
