...
own pixel dimensions with no resize hook, so a source pack meant for a full-size button prompt
reads as oversized and misaligned inline in a line of text; pre-scaling the art is the fix until
Bevy grows one of its own. Needs `magick` (ImageMagick) on PATH.

`make_inline` resizes color and alpha separately rather than handing the whole RGBA image to one
`-resize`: the source stores white under full transparency, and a filter that blends neighboring
pixels' *stored* color irrespective of their alpha bleeds that white into the resized edge as a
visible halo — the classic unpremultiplied-alpha resize artifact. Compositing over black first
turns invisible-but-white into invisible-but-black before the blend, which a straight-alpha PNG
consumer never sees since a fully-transparent pixel's color never draws either way.
"""
...
DST_INLINE = "assets/input_prompts_inline"
INLINE_SIZE = 25
...
def make_inline(src, dst):
    """Writes a downscaled, alpha-correct copy of `src` to `dst` — see the module doc for why this
    is five `magick` calls rather than one `-resize`."""
    tmp = {name: dst + "." + name + ".png" for name in ("rgb", "alpha", "rgb_s", "alpha_s", "comp")}
    try:
        subprocess.run(
            ["magick", src, "-background", "black", "-alpha", "remove", "-alpha", "off", tmp["rgb"]],
            check=True,
        )
        subprocess.run(["magick", src, "-alpha", "extract", tmp["alpha"]], check=True)
        size = "%dx%d" % (INLINE_SIZE, INLINE_SIZE)
        subprocess.run(["magick", tmp["rgb"], "-filter", "Lanczos", "-resize", size, tmp["rgb_s"]], check=True)
        subprocess.run(["magick", tmp["alpha"], "-filter", "Lanczos", "-resize", size, tmp["alpha_s"]], check=True)
        subprocess.run(
            ["magick", tmp["rgb_s"], tmp["alpha_s"], "-compose", "CopyOpacity", "-composite", tmp["comp"]],
            check=True,
        )
        # Un-premultiplies: `comp` is color-times-alpha from the compositing above, and a straight-
...
for tier, control, stem in jobs:
    ...
    dst_inline = os.path.join(DST_INLINE, tier, control + ".png")
    os.makedirs(os.path.dirname(dst), exist_ok=True)
    os.makedirs(os.path.dirname(dst_inline), exist_ok=True)
    if "--apply" in sys.argv:
        shutil.copy2(src, dst)
        make_inline(src, dst_inline)
    copied += 1
...
if "--apply" in sys.argv:
    ...
    with open(os.path.join(DST, "manifest.txt"), "w") as f:
        ...
        f.write("# the next tier, and a missing path is never a load error. Covers input_prompts_inline/\n")
        f.write("# too — every entry here has a downscaled copy there as well.\n")
        for e in entries:
            f.write(e + "\n")
    print("wrote %s/manifest.txt with %d entries" % (DST, len(entries)))
    shutil.copy2(os.path.join(DST, "LICENSE.txt"), os.path.join(DST_INLINE, "LICENSE.txt"))
