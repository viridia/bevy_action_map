...
| [Roadmap.md](./Roadmap.md) | The build order — 14 chunks in three phases, each meant to be reviewable on its own. **Start here to write code.** |
...
Use `--bevy` to measure a pad's **resting drift**, which is the D6 stage-1 calibration number that
Roadmap chunk 8 needs and that cannot be read in the default mode.
