...
//!
//! An [`InputContextState`] is the live runtime state for one gameplay context. Use one when you
//! want the same bindings and the same action reads, but separate state for different players,
...
/// [`add_context`](ActionMapAppExt::add_context). It installs the input frame sampler if you have
/// not added [`InputFramePlugin`](crate::frame::InputFramePlugin) yourself, and orders context
...
impl Plugin for ActionMapPlugin {
    fn build(&self, app: &mut App) {
        ...
        if !app.is_plugin_added::<crate::frame::InputFramePlugin>() {
            app.add_plugins(crate::frame::InputFramePlugin);
        }
